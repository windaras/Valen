...
<div class="overlay">
  <div class="center-message">EVELYN ME ENCABTAS Y TE QUIEROOO MUCHO 💖</div> <!-- Corazón rosa -->
</div>
...
<script>
  ...
  const handleInteraction = (e) => {
    ...
    directions.forEach(dir => {
      particles.push({
        ...
        color: `#${Math.random() > 0.5 ? 'ff69b4' : 'ff007f'}` /* Alterna entre rosas */
      });
    });
  ...
  function drawParticles() {
    particles.forEach((p, index) => {
      ...
      ctx.shadowBlur = 15;
      ctx.fillText("ME ENCANTAS", p.x, p.y);
      ctx.shadowBlur = 0;
      p.x += p.vx;
      p.y += p.vy;
      p.alpha -= 0.005;
